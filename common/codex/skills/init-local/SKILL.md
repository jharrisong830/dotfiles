---
name: init-local
description: Generate an AGENTS.override.md for the current project. Use this to initialize local-only Codex context that should not be committed to version control.
argument-hint: [any extra notes or context to include]
---

Analyze the current project and generate an `AGENTS.override.md` file in the project root. This file is for local-only Codex context and must not be committed.

Follow these steps:

1. Explore the project structure: identify the language, runtime, build tool, test framework, and key entry points.
2. Read any existing README, `AGENTS.override.md`, or documentation to understand the project's purpose.
3. Check for existing `AGENTS.md` and `AGENTS.override.md` files in the project root. Read them, and do not duplicate any context or instructions already covered there in the generated `AGENTS.override.md`.
4. Check whether `AGENTS.override.md` already exists:
   - **First run (file does not exist):** Write `AGENTS.override.md` from scratch using the structure below.
...
5. Check `.gitignore` — if `AGENTS.override.md` is not already listed, add it. If no `.gitignore` exists, create one with that entry.

Do not create or modify `AGENTS.md`.

## AGENTS.override.md structure
